"""
This module solves Part One of Day 19's problem of the Advent of Code challenge.
No major tricks here - one nice move was having the rule follower call itself recursively to
bubble up the final results of the rule chain back to the original caller.
"""
# --- Day 19: Aplenty ---
...
# (A). If a part is sent to another workflow, it immediately switches to the start of that workflow
...
def parse_input(filename):
    ...
    rules_list, parts_list = data[:data.index('\n')], data[data.index('\n')+1:]
    ...
    for line in parts_list:
        line = line.strip('{}\n').split(',')
        values = [int(item.split('=')[1]) for item in line]
        x, m, a, s = values
        part = {}
        part['x'] = x
        part['m'] = m
    ...
    ic(part_list,rule_dict)
    return part_list, rule_dict

def run_instructions(part_list, rule_dict):
    """ Run a set of instructions on a list of parts """
    accepted_parts = []
    for part in part_list:
        result = run_instruction(part, rule_dict, rule_dict['in'])
        if result == 'A':
            accepted_parts.append(part)

    sum_accepted_values = 0
    # Sum values of accepted parts
    for part in accepted_parts:
        for key in ['x','m','a','s']:
            sum_accepted_values += part[key]
            
    return ic(sum_accepted_values)

def run_instruction(part, rule_dict, rules):
    """ Follow a given rule recursively to determine if a part is accepted or rejected """
    for rule in rules:
        # Base case: immediate accept or reject
        if rule == 'A':
            return 'A'
        if rule == 'R':
            return 'R'
        
        # Case: immediate route to a label
        if rule in rule_dict.keys():
            return run_instruction(part, rule_dict, rule_dict[rule])
        
        # Case: need to check if part satisfies rule
        test_destination = rule.split(':')[1]
        test_value = rule.split(':')[0][2:]
        test_label = rule[0] # x, m, a, or s
        test_operator = rule[1]
        
        value = part[test_label]
        if (test_operator == '<' and value < int(test_value)) \
            or (test_operator == '>' and value > int(test_value)):
            if test_destination == 'A':
                return 'A'
            if test_destination == 'R':
                return 'R'
            return run_instruction(part, rule_dict, rule_dict[test_destination])

def main():
    """
    Reads in the input file, processes and outputs the solution
    """
    part_list, rule_dict = parse_input("input.txt")
    
    run_instructions(part_list, rule_dict)
